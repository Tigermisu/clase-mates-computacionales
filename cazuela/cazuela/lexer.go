...
func scanNextToken() {
	...
	default:
		if isDigit(character) {
			...
			HaltExecutionWithError(MenudoError{CodeSyntaxError,
				line, fmt.Sprintf("Unexpected character %c", character), ""})
		}
...
func parseStringLexeme() {
	...
	if atEndOfCommand() {
		RaiseError(CodeUnexpectedEOF, "Se esperaba terminar una cadena, pero el archivo se acabó.", line, "", true)
	} else {
		currentPosition++
